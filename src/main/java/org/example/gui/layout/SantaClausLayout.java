package org.example.gui.layout;

import org.example.entity.Place;

/**
 * <p>Title: Ant-Mahjongg</p>
 * <p>Description: </p>
 * <p>Copyright: Copyright (c) 2004-2005</p>
 * <p>Company: </p>
 *
 * @author Smigura Antal
 * @version 1.2
 */

public class SantaClausLayout extends Layout {

    public void init() {
        // 0. réteg
        // 0. sor
        places.add(new Place(2.5, 0, 0));
        places.add(new Place(3.5, 0, 0));
        places.add(new Place(4.5, 0, 0));
        places.add(new Place(5.5, 0, 0));
        places.add(new Place(6.5, 0, 0));

        // 0,5. sor
        places.add(new Place(7.5, 0.5, 0));

        // 1. sor
        places.add(new Place(1.5, 1, 0));
        places.add(new Place(2.5, 1, 0));
        places.add(new Place(3.5, 1, 0));
        places.add(new Place(4.5, 1, 0));
        places.add(new Place(5.5, 1, 0));

        places.add(new Place(8.5, 1, 0));

        // 1,5. sor
        places.add(new Place(9.5, 1.5, 0));

        // 2. sor
        places.add(new Place(0.5, 2, 0));

        places.add(new Place(6.5, 2, 0));

        // 2,5. sor
        places.add(new Place(2.5, 2.5, 0));

        places.add(new Place(4.5, 2.5, 0));

        places.add(new Place(9.5, 2.5, 0));

        // 3. sor
        places.add(new Place(0, 3, 0));

        places.add(new Place(7, 3, 0));

        // 3,5. sor
        places.add(new Place(3.5, 3.5, 0)); // orra

        // 4. sor
        places.add(new Place(1, 4, 0));

        places.add(new Place(6, 4, 0));

        // 4,5. sor
        places.add(new Place(0, 4.5, 0));

        places.add(new Place(2, 4.5, 0));

        places.add(new Place(5, 4.5, 0));

        places.add(new Place(7, 4.5, 0));

        // 5. sor
        places.add(new Place(1, 5, 0));

        places.add(new Place(3, 5, 0));
        places.add(new Place(4, 5, 0));

        places.add(new Place(6, 5, 0));

        // 5,5. sor
        places.add(new Place(0, 5.5, 0));

        places.add(new Place(2, 5.5, 0));

        places.add(new Place(5, 5.5, 0));

        places.add(new Place(7, 5.5, 0));

        // 6. sor
        places.add(new Place(1, 6, 0));

        places.add(new Place(3, 6, 0));
        places.add(new Place(4, 6, 0));

        places.add(new Place(6, 6, 0));

        // 6,5. sor
        places.add(new Place(2, 6.5, 0));

        places.add(new Place(5, 6.5, 0));

        // 7. sor
        places.add(new Place(3, 7, 0));
        places.add(new Place(4, 7, 0));

        // 1. réteg
        // 0. sor
        places.add(new Place(2.5, 0, 1));
        places.add(new Place(3.5, 0, 1));
        places.add(new Place(4.5, 0, 1));
        places.add(new Place(5.5, 0, 1));
        places.add(new Place(6.5, 0, 1));

        // 0,5. sor
        places.add(new Place(7.5, 0.5, 1));

        // 1. sor
        places.add(new Place(1.5, 1, 1));
        places.add(new Place(2.5, 1, 1));
        places.add(new Place(3.5, 1, 1));
        places.add(new Place(4.5, 1, 1));
        places.add(new Place(5.5, 1, 1));

        places.add(new Place(8.5, 1, 1));

        // 1,5. sor
        places.add(new Place(9.5, 1.5, 1));

        // 2. sor
        places.add(new Place(0.5, 2, 1));

        places.add(new Place(6.5, 2, 1));

        // 2,5. sor
        places.add(new Place(2.5, 2.5, 1));

        places.add(new Place(4.5, 2.5, 1));

        // 3. sor
        places.add(new Place(0, 3, 1));

        places.add(new Place(7, 3, 1));

        // 3,5. sor
        places.add(new Place(3.5, 3.5, 1)); // orra

        // 4. sor
        places.add(new Place(1, 4, 1));

        places.add(new Place(6, 4, 1));

        // 4,5. sor
        places.add(new Place(0, 4.5, 1));

        places.add(new Place(2, 4.5, 1));

        places.add(new Place(5, 4.5, 1));

        places.add(new Place(7, 4.5, 1));

        // 5. sor
        places.add(new Place(1, 5, 1));

        places.add(new Place(3, 5, 1));
        places.add(new Place(4, 5, 1));

        places.add(new Place(6, 5, 1));

        // 5,5. sor
        places.add(new Place(0, 5.5, 1));

        places.add(new Place(2, 5.5, 1));

        places.add(new Place(5, 5.5, 1));

        places.add(new Place(7, 5.5, 1));

        // 6. sor
        places.add(new Place(1, 6, 1));

        places.add(new Place(3, 6, 1));
        places.add(new Place(4, 6, 1));

        places.add(new Place(6, 6, 1));

        // 6,5. sor
        places.add(new Place(2, 6.5, 1));

        places.add(new Place(5, 6.5, 1));

        // 7. sor
        places.add(new Place(3, 7, 1));
        places.add(new Place(4, 7, 1));

        // 2. réteg
        // 0. sor
        places.add(new Place(2.5, 0, 2));
        places.add(new Place(3.5, 0, 2));
        places.add(new Place(4.5, 0, 2));

        // 1. sor
        places.add(new Place(1.5, 1, 2));
        places.add(new Place(2.5, 1, 2));
        places.add(new Place(3.5, 1, 2));
        places.add(new Place(4.5, 1, 2));
        places.add(new Place(5.5, 1, 2));

        // 2. sor
        places.add(new Place(0.5, 2, 2));

        places.add(new Place(6.5, 2, 2));

        // 2,5. sor
        places.add(new Place(2.5, 2.5, 2));

        places.add(new Place(4.5, 2.5, 2));

        // 3. sor
        places.add(new Place(0, 3, 2));

        places.add(new Place(7, 3, 2));

        // 3,5. sor
        places.add(new Place(3.5, 3.5, 2)); // orra

        // 4,5. sor
        places.add(new Place(0.5, 4.5, 2));
        places.add(new Place(1.5, 4.5, 2));

        places.add(new Place(5.5, 4.5, 2));
        places.add(new Place(6.5, 4.5, 2));

        // 5. sor
        places.add(new Place(2.5, 5, 2));
        places.add(new Place(3.5, 5, 2));
        places.add(new Place(4.5, 5, 2));

        // 5,5. sor
        places.add(new Place(0.5, 5.5, 2));
        places.add(new Place(1.5, 5.5, 2));

        places.add(new Place(5.5, 5.5, 2));
        places.add(new Place(6.5, 5.5, 2));

        // 6. sor
        places.add(new Place(2.5, 6, 2));
        places.add(new Place(3.5, 6, 2));
        places.add(new Place(4.5, 6, 2));

        // 7. sor
        places.add(new Place(3, 7, 2));
        places.add(new Place(4, 7, 2));

        // 3. réteg
        // 2. sor
        places.add(new Place(0.5, 2, 3));

        places.add(new Place(6.5, 2, 3));

        // 2,5. sor
        places.add(new Place(2.5, 2.5, 3));

        places.add(new Place(4.5, 2.5, 3));

        // 3. sor
        places.add(new Place(0, 3, 3));

        places.add(new Place(7, 3, 3));

        // 3,5. sor
        places.add(new Place(3.5, 3.5, 3)); // orra

        // 4,5. sor
        places.add(new Place(0.5, 4.5, 3));
        places.add(new Place(1.5, 4.5, 3));

        places.add(new Place(5.5, 4.5, 3));
        places.add(new Place(6.5, 4.5, 3));

        // 5. sor
        places.add(new Place(2.5, 5, 3));
        places.add(new Place(3.5, 5, 3));
        places.add(new Place(4.5, 5, 3));

        // 5,5. sor
        places.add(new Place(0.5, 5.5, 3));
        places.add(new Place(1.5, 5.5, 3));

        places.add(new Place(5.5, 5.5, 3));
        places.add(new Place(6.5, 5.5, 3));

        // 6. sor
        places.add(new Place(2.5, 6, 3));
        places.add(new Place(3.5, 6, 3));
        places.add(new Place(4.5, 6, 3));

        // 7. sor
        places.add(new Place(3, 7, 3));
        places.add(new Place(4, 7, 3));

        // 4. réteg
        // 4,5. sor
        places.add(new Place(1.5, 4.5, 4));

        places.add(new Place(5.5, 4.5, 4));

        // 5. sor
        places.add(new Place(2.5, 5, 4));
        places.add(new Place(3.5, 5, 4));
        places.add(new Place(4.5, 5, 4));

        // méret
        layoutWidth = 10.5;
        layoutHeight = 8;
    }

    public String getName() {
        return "Santa Claus";
    }

}