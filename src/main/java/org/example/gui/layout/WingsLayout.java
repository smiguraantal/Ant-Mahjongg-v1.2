package org.example.gui.layout;

import org.example.entity.Place;

/**
 * <p>Title: Ant-Mahjongg</p>
 * <p>Description: </p>
 * <p>Copyright: Copyright (c) 2004-2005</p>
 * <p>Company: </p>
 *
 * @author Smigura Antal
 * @version 1.2
 */

public class WingsLayout extends Layout {

    public void init() {
        // 0. réteg
        // 0. sor
        places.add(new Place(0, 0, 0));

        places.add(new Place(14, 0, 0));

        // 0,5. sor
        places.add(new Place(1, 0.5, 0));

        places.add(new Place(13, 0.5, 0));

        // 1. sor
        places.add(new Place(2, 1, 0));

        places.add(new Place(12, 1, 0));

        // 1,5. sor
        places.add(new Place(0, 1.5, 0));

        places.add(new Place(3, 1.5, 0));

        places.add(new Place(11, 1.5, 0));

        places.add(new Place(14, 1.5, 0));

        // 2. sor
        places.add(new Place(1, 2, 0));

        places.add(new Place(4, 2, 0));

        places.add(new Place(10, 2, 0));

        places.add(new Place(13, 2, 0));

        // 2,5. sor
        places.add(new Place(2, 2.5, 0));

        places.add(new Place(5, 2.5, 0));

        places.add(new Place(9, 2.5, 0));

        places.add(new Place(12, 2.5, 0));

        // 3. sor
        places.add(new Place(0, 3, 0));

        places.add(new Place(3, 3, 0));

        places.add(new Place(6, 3, 0));
        places.add(new Place(8, 3, 0));

        places.add(new Place(11, 3, 0));

        places.add(new Place(14, 3, 0));

        // 3,5. sor
        places.add(new Place(1, 3.5, 0));

        places.add(new Place(4, 3.5, 0));

        places.add(new Place(7, 3.5, 0));

        places.add(new Place(10, 3.5, 0));

        places.add(new Place(13, 3.5, 0));

        // 4. sor
        places.add(new Place(2, 4, 0));

        places.add(new Place(5, 4, 0));

        places.add(new Place(9, 4, 0));

        places.add(new Place(12, 4, 0));

        // 4,5. sor
        places.add(new Place(0, 4.5, 0));

        places.add(new Place(3, 4.5, 0));

        places.add(new Place(6, 4.5, 0));
        places.add(new Place(8, 4.5, 0));

        places.add(new Place(11, 4.5, 0));

        places.add(new Place(14, 4.5, 0));

        // 5. sor
        places.add(new Place(1, 5, 0));

        places.add(new Place(4, 5, 0));

        places.add(new Place(7, 5, 0));

        places.add(new Place(10, 5, 0));

        places.add(new Place(13, 5, 0));

        // 5,5. sor
        places.add(new Place(2, 5.5, 0));

        places.add(new Place(5, 5.5, 0));

        places.add(new Place(9, 5.5, 0));

        places.add(new Place(12, 5.5, 0));

        // 6. sor
        places.add(new Place(3, 6, 0));

        places.add(new Place(6, 6, 0));

        places.add(new Place(8, 6, 0));

        places.add(new Place(11, 6, 0));

        // 6,5. sor
        places.add(new Place(4, 6.5, 0));

        places.add(new Place(7, 6.5, 0));

        places.add(new Place(10, 6.5, 0));

        // 7. sor
        places.add(new Place(5, 7, 0));

        places.add(new Place(9, 7, 0));

        // 7,5. sor
        places.add(new Place(6, 7.5, 0));
        places.add(new Place(8, 7.5, 0));

        // 8. sor
        places.add(new Place(7, 8, 0));

        // 1. réteg
        // 0. sor
        places.add(new Place(0, 0, 1));

        places.add(new Place(14, 0, 1));

        // 0,5. sor
        places.add(new Place(1, 0.5, 1));

        places.add(new Place(13, 0.5, 1));

        // 1. sor
        places.add(new Place(2, 1, 1));

        places.add(new Place(12, 1, 1));

        // 1,5. sor
        places.add(new Place(0, 1.5, 1));

        places.add(new Place(3, 1.5, 1));

        places.add(new Place(11, 1.5, 1));

        places.add(new Place(14, 1.5, 1));

        // 2. sor
        places.add(new Place(1, 2, 1));

        places.add(new Place(4, 2, 1));

        places.add(new Place(10, 2, 1));

        places.add(new Place(13, 2, 1));

        // 2,5. sor
        places.add(new Place(2, 2.5, 1));

        places.add(new Place(5, 2.5, 1));

        places.add(new Place(9, 2.5, 1));

        places.add(new Place(12, 2.5, 1));

        // 3. sor
        places.add(new Place(0, 3, 1));

        places.add(new Place(3, 3, 1));

        places.add(new Place(6, 3, 1));
        places.add(new Place(8, 3, 1));

        places.add(new Place(11, 3, 1));

        places.add(new Place(14, 3, 1));

        // 3,5. sor
        places.add(new Place(1, 3.5, 1));

        places.add(new Place(4, 3.5, 1));

        places.add(new Place(7, 3.5, 1));

        places.add(new Place(10, 3.5, 1));

        places.add(new Place(13, 3.5, 1));

        // 4. sor
        places.add(new Place(2, 4, 1));

        places.add(new Place(5, 4, 1));

        places.add(new Place(9, 4, 1));

        places.add(new Place(12, 4, 1));

        // 4,5. sor
        places.add(new Place(0, 4.5, 1));

        places.add(new Place(3, 4.5, 1));

        places.add(new Place(6, 4.5, 1));
        places.add(new Place(8, 4.5, 1));

        places.add(new Place(11, 4.5, 1));

        places.add(new Place(14, 4.5, 1));

        // 5. sor
        places.add(new Place(1, 5, 1));

        places.add(new Place(4, 5, 1));

        places.add(new Place(7, 5, 1));

        places.add(new Place(10, 5, 1));

        places.add(new Place(13, 5, 1));

        // 5,5. sor
        places.add(new Place(2, 5.5, 1));

        places.add(new Place(5, 5.5, 1));

        places.add(new Place(9, 5.5, 1));

        places.add(new Place(12, 5.5, 1));

        // 6. sor
        places.add(new Place(3, 6, 1));

        places.add(new Place(6, 6, 1));
        places.add(new Place(8, 6, 1));

        places.add(new Place(11, 6, 1));

        // 6,5. sor
        places.add(new Place(4, 6.5, 1));

        places.add(new Place(7, 6.5, 1));

        places.add(new Place(10, 6.5, 1));

        // 7. sor
        places.add(new Place(5, 7, 1));

        places.add(new Place(9, 7, 1));

        // 7,5. sor
        places.add(new Place(6, 7.5, 1));

        places.add(new Place(8, 7.5, 1));

        // 8. sor
        places.add(new Place(7, 8, 1));

        // 2. réteg
        // 0. sor
        places.add(new Place(0, 0, 2));

        places.add(new Place(14, 0, 2));

        // 1. sor
        places.add(new Place(2, 1, 2));

        places.add(new Place(12, 1, 2));

        // 1,5. sor
        places.add(new Place(0, 1.5, 2));

        places.add(new Place(14, 1.5, 2));

        // 2,5. sor
        places.add(new Place(2, 2.5, 2));

        places.add(new Place(12, 2.5, 2));

        // 3. sor
        places.add(new Place(0, 3, 2));

        places.add(new Place(14, 3, 2));

        // 4. sor
        places.add(new Place(2, 4, 2));

        places.add(new Place(12, 4, 2));

        // 4,5. sor
        places.add(new Place(0, 4.5, 2));

        places.add(new Place(14, 4.5, 2));

        // 5. sor
        places.add(new Place(4, 5, 2));

        places.add(new Place(10, 5, 2));

        // 5,5. sor
        places.add(new Place(2, 5.5, 2));

        places.add(new Place(12, 5.5, 2));

        // 6. sor
        places.add(new Place(6, 6, 2));
        places.add(new Place(8, 6, 2));

        // 6,5. sor
        places.add(new Place(4, 6.5, 2));

        places.add(new Place(10, 6.5, 2));

        // 7,5. sor
        places.add(new Place(6, 7.5, 2));

        places.add(new Place(8, 7.5, 2));

        // méret
        layoutWidth = 15;
        layoutHeight = 9;
    }

    public String getName() {
        return "Wings";
    }

}


