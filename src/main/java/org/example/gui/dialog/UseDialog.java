...
public class UseDialog extends JDialog {
    ...
    private DefaultMutableTreeNode root = new DefaultMutableTreeNode("Mahjongg");
    ...
    private DefaultMutableTreeNode nodeMenu =
            new DefaultMutableTreeNode("Menüpontok");
    private DefaultMutableTreeNode nodePenalty =
            new DefaultMutableTreeNode("Büntetés");
    private DefaultMutableTreeNode nodeEnd =
            new DefaultMutableTreeNode("És végül...");
     ...
     * Konstruktor
     */
    public UseDialog(MainFrame mainFrame) {
    ...
    private void jbInit() throws Exception {
        ...
        scrollEditor.setPreferredSize(new Dimension(450, 300));
        ...
        treeModel.insertNodeInto(nodeMenu, root, root.getChildCount());
        treeModel.insertNodeInto(nodePenalty, root, root.getChildCount());
        treeModel.insertNodeInto(nodeShortcuts, root, root.getChildCount());
        treeModel.insertNodeInto(nodeEnd, root, root.getChildCount());
    ...
    private String createPath(DefaultMutableTreeNode selectedNode) {
        ...
        if (selectedNode == nodeRules)
            file = "rules.html";
        if (selectedNode == nodeMenu)
            file = "menu.html";
        if (selectedNode == nodePenalty)
            file = "penalty.html";
        if (selectedNode == nodeEnd)
            file = "end.html";
